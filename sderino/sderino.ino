...
void setup() {
  ...
  Music = SD.open("music.wav",FILE_READ);
  //Se comprueba que el archivo se ha abierto correctamente y se procede a
  //escribir en él.

  if (Music){
    Serial.println("Music leído");
    Music.println
    }

  else{
    Serial.println("FALLO ESTA MIERDA");
    }
  ...
  if (Archivo) {
    ...
    Archivo.println("Esto es lo que se esta escribiendo en el archivo");
